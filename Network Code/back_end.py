...
def create_model(num_input_channels,input_shapes,num_LSTM_layers,num_LSTM_units,num_dense_layers,num_dense_units,learning_rate,reg_param,inp_optim):
    ...
    model.compile(optimizer=inp_optim,loss="categorical_crossentropy")
    model.summary()
...
def train_model(batch_sz,eps,val_splt,model,input_list,output_list,exp_no):
    ...
    checkpoint = ModelCheckpoint('weights.best.hdf5', monitor='train_loss', verbose=1, save_best_only=True, mode='min')
