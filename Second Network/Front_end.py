...
for i_inp in range(num_input_columns):
    if i_inp==2 or i_inp==0:  # Hard Coded value: colum corresponding to crew size
        continue;
...
num_dense_layers=2;
num_dense_units=[32,5]; # Last dense units always has to be number of risk categories (5)
learning_rate=10**(-4);
inp_optim="Adam";
reg_param=0;
batch_sz=4096;
eps=300;
val_splt=0.10;
output_size_embedding=16 # Hyper-parameter unoptimized
...
model=back_end.create_model(num_input_channels,vocab_sizes,output_size_embedding,input_shapes,num_LSTM_layers,num_LSTM_units,num_dense_layers,num_dense_units,learning_rate,reg_param,inp_optim)
