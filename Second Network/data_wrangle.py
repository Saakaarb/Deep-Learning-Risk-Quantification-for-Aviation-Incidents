...
'''
print("Self replicating data with low class representation")
input_data=input_data.values.tolist()
i=1
while i < (len(risk_data)):
    ...
    if risk_data[i]!=3:
        ...
        for j in range(len(input_data)):
            ...
            input_data[j].insert(i,input_data[j])
        risk_data.insert(i,risk_data[i])
        narrative_data_main.insert(i,narrative_data_main[i])
        i=i+1
    i=i+1
...
print("Shuffle,then save narrative and input data using pickle, and encoded risk matrix")
...
with open('narrative_data_test', 'wb') as f:
    pickle.dump(narrative_data_main, f)
